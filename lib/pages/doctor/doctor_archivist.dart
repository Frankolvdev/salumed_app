...
import 'package:app/components/select_open_pdf_dialog.dart';
...
class DoctorArchivist extends StatefulWidget {
  const DoctorArchivist({Key? key}) : super(key: key);

  @override
  State<DoctorArchivist> createState() => _DoctorArchivistState();
}

class _DoctorArchivistState extends State<DoctorArchivist> {
  ...
  Future<Null> loadMore() {
    ...
    return WebService(context)
        .getArchives(limit, archives.length, context, provider.user.id ?? "",
            provider.user.token ?? "",
            search: cSearch.text)
        .then((value) {
      if (value.length > 0) {
  ...
  Future<Null> load() {
    final provider = Provider.of<AppProvider>(context, listen: false);
    return WebService(context)
        .getArchives(limit, 0, context, provider.user.id ?? "",
            provider.user.token ?? "",
            search: cSearch.text)
        .then((value) {
      if (mounted)
          ...
          return CustomDialog(
            ...
            () {
              simpleLoading(context, (BuildContext loadingContext) {
                ...
                WebService(context)
                    .deleteArchive(provider.user.token ?? "", archive.id ?? "")
                    .then((res) {
                  SnackBar(
