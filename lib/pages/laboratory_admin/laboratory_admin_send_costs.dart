...
class laboratoryAdminSendCosts extends StatefulWidget {
  Function callBackBack;
  OrderModel order;
  laboratoryAdminSendCosts(this.callBackBack, this.order, {Key? key})
      : super(key: key);

  @override
  _laboratoryAdminSendCostsState createState() =>
      _laboratoryAdminSendCostsState();
}

class _laboratoryAdminSendCostsState extends State<laboratoryAdminSendCosts> {
  ...
  void initState() {
    ...
    if (lastBudget != null) {
      ...
      budgetTmp.medicines.forEach((element) {
        ...
        bool exist = false;
        medicines.forEach((elementExist) {
          if (elementExist.medicineDefault == element.medicine) {
            exist = true;
          }
        });
      ...
      if (lastBudget == null) {
        studiesValues.forEach((element) {
          ...
          medicines.add(RowTableMedicine(
            ...
            {"medicine": "", "cost": 0, "amount": 0, "quantity": 0},
            false,
            key: _scaffoldKey,
          ));
        });
  ...
  bool checkTable() {
    ...
    if (medicines.length <= 0) {
      flag = false;
      showErrorsDialog(
          context, ["Debe agregar al menos 1 estudio a la tabla de costos"]);
      return flag;
    }
    medicines.forEach((row) {
    ...
    if (!flag) {
      showErrorsDialog(context, [
        "Verifique que el campo nombre de estudio no este vacio",
