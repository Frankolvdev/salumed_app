...
import 'package:app/components/view_select_map.dart';
...
class LaboratoryAdminEditLaboratory extends StatefulWidget {
  LaboratoryAdminEditLaboratory({Key? key}) : super(key: key);

  @override
  _LaboratoryAdminEditLaboratoryState createState() =>
      _LaboratoryAdminEditLaboratoryState();
}

class _LaboratoryAdminEditLaboratoryState
    extends State<LaboratoryAdminEditLaboratory> {
  ...
  void initState() {
    ...
    PharmacyModel p = provider.user.pharmacy_assigned!;
  ...
  loadAdmins() {
    ...
    return WebService(context)
        .getUsers(0, 0, context, provider.user.token ?? "",
            search: "", filter_rol: "laboratory_admin")
        .then((value) {
      if (mounted)
        setState(() {
          admins = value;
  ...
  Widget build(BuildContext context) {
    ...
    PharmacyModel pharmacy = provider.user.pharmacy_assigned!;
          ...
          suffixIcon: InkWell(
            onTap: () {
              Navigator.push(
                  context,
                  PageTransition(
                      child: ViewSelectMap(placeSelected),
        ...
        try {
          ...
          PharmacyModel pharmacyTmp = await WebService(context).updatePharmacy(
              ...
              (placeSelected != null && placeSelected is Place)
                  ...
                  : "",
              provider.user.token ?? "");
          provider.user.pharmacy_assigned = pharmacyTmp;
