...
import 'package:app/pages/hospital_admin/hospital_admin_view_prescription.dart';
...
class HospitalAdminPrescriptions extends StatefulWidget {
  Function callbackToOrders;
  HospitalAdminPrescriptions(this.callbackToOrders, {Key? key})
      : super(key: key);

  @override
  _HospitalAdminPrescriptionsState createState() =>
      _HospitalAdminPrescriptionsState();
}

class _HospitalAdminPrescriptionsState
    extends State<HospitalAdminPrescriptions> {
  ...
  Future<Null> loadMore() {
    ...
    return WebService(context)
        .getPrescription(
            limit, prescriptions.length, context, provider.user.token ?? "",
            search: cSearch.text, id_user: provider.user.id ?? "")
        .then((value) {
      if (value.length > 0) {
        if (mounted)
          setState(() {
  ...
  Future<Null> load() {
    final provider = Provider.of<AppProvider>(context, listen: false);
    return WebService(context)
        .getPrescription(limit, 0, context, provider.user.token ?? "",
            search: cSearch.text, id_user: provider.user.id ?? "")
        .then((value) {
      if (mounted)
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    if (viewPrescription != null) {
      return HospitalAdminViewPrescription((val) {
    ...
    List<ResponsiveGridCol> progressWidgets =
        prescriptions.asMap().entries.map((prescription) {
      PrescriptionModel pTmp = prescription.value;
                              ...
                              child: Column(
                                children: [
                                  Container(
                                    ...
                                    child: (pTmp.patient!.picture != null &&
                                            getImageUrl(pTmp.patient!.picture!)
                                                    .trim() !=
                                                "")
                                        ? ClipRRect(
                                            borderRadius:
                              ...
                              child: Column(
                                children: [
                                  Container(
                                    ...
                                    child: (pTmp.doctor != null &&
                                            pTmp.doctor!.picture != null &&
                                            getImageUrl(pTmp.doctor!.picture!)
                                                    .trim() !=
                                                "")
                                        ? ClipRRect(
                                            borderRadius:
                                  ...
                                  Padding(
                                    ...
                                    child: Container(
                                      ...
                                      child: Padding(
                                        padding: const EdgeInsets.all(5.0),
                                        child: Text(
                                            "Medicamentos aún no solicitados",
                                            style: TextStyle(
                                                fontWeight: FontWeight.normal,
                                                color: Colors.white,
                                                fontSize: 14),
