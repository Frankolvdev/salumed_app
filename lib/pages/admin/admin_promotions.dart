...
import 'package:app/pages/admin/admin_add_promotion.dart';
import 'package:app/pages/admin/admin_add_user.dart';
import 'package:app/pages/admin/admin_adit_promotion.dart';
...
class AdminPromotions extends StatefulWidget {
  const AdminPromotions({Key? key}) : super(key: key);
...
class _AdminPromotionsState extends State<AdminPromotions> {
  ...
  Future<Null> loadMore() {
    ...
    return WebService(context)
        .getPromotions(
            limit, promotions.length, context, provider.user.token ?? "",
            search: cSearch.text)
        .then((value) {
      if (value.length > 0) {
        if (mounted)
  ...
  Future<Null> load() {
    final provider = Provider.of<AppProvider>(context, listen: false);
    return WebService(context)
        .getPromotions(limit, 0, context, provider.user.token ?? "",
            search: cSearch.text)
        .then((value) {
      if (mounted)
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    List<ResponsiveGridCol> progressWidgets =
        promotions.asMap().entries.map((p) {
      PromotionModel pTmp = p.value;
          ...
          return CustomDialog(
            ...
            () {
              simpleLoading(context, (BuildContext loadingContext) {
                ...
                WebService(context)
                    .deletePromotion(provider.user.token ?? "", promo.id ?? "")
                    .then((res) {
                  SnackBar(
