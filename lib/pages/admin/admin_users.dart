...
class _AdminUsersState extends State<AdminUsers> {
    ...
    return Scaffold(
        ...
        body: Stack(
          children: [
            ...
            Positioned.fill(
                top: (MediaQuery.of(context).padding.top) + 245,
  ...
  notify() {
    showDialog(
        ...
        builder: (contextDialog) {
          return NotifyDialog(
            (contextDialogForm, title, message, rol, notification, email) {
              simpleLoading(context, (BuildContext loadingContext) {
                final provider =
                    Provider.of<AppProvider>(context, listen: false);

                WebService(context)
                    .notify(title, message, rol, notification, email, [],
                        provider.user.token ?? "")
                    .then((res) {
                  SnackBar(
                  ...
                  Navigator.pop(loadingContext);
                  Navigator.pop(contextDialogForm);
                }).catchError((e) {
                  Navigator.pop(loadingContext);
                  showErrorsDialog(context, e);
                });
  ...
  notifyUsers(List<String> users) {
    showDialog(
        barrierDismissible: false,
        context: context,
        builder: (contextDialog) {
          return NotifyUsersDialog(
            (contextDialogForm, title, message, rol, notification, email) {
              simpleLoading(context, (BuildContext loadingContext) {
                final provider =
                    Provider.of<AppProvider>(context, listen: false);
                ...
                WebService(context)
                    .notify(title, message, rol, notification, email, users,
                        provider.user.token ?? "")
                    .then((res) {
                  SnackBar(
                          content: Text("Se ha enviado correctamente",
