...
class GuestPrescription extends StatefulWidget {
  Function callBackBack;
  dynamic dataGuest;
  GuestPrescription(this.callBackBack, this.dataGuest, {Key? key})
      : super(key: key);

  @override
  _GuestPrescriptionState createState() => _GuestPrescriptionState();
}

class _GuestPrescriptionState extends State<GuestPrescription> {
  ...
  void initState() {
    ...
    tokenDoctor = (widget.dataGuest["doctor_guest"] as UserModel).token!;
    cEmail.text = widget.dataGuest["email_guest"] ?? "";
    WidgetsBinding.instance!.addPostFrameCallback((_) {
      setSelectPatient((widget.dataGuest["patient"] as UserModel));
    });
  }
  ...
  setSelectPatient(UserModel patient) {
    setState(() {
      patientSelected = patient;
      cPatient.text = patient.name ?? "Sin nombre";

      cYears.text = (patient.years != null && patient.years != "")
          ? patient.years ?? ""
          : "";
      cBloodType.text = (patient.blood_type != null && patient.blood_type != "")
          ? patient.blood_type ?? ""
          : "";
      cOrganDonor.text =
          (patient.organ_donor != null && patient.organ_donor != "")
              ? patient.organ_donor ?? ""
              : "";
      cDiseases.text = (patient.diseases != null && patient.diseases != "")
          ? patient.diseases ?? ""
          : "";
      cAllergies.text = (patient.allergies != null && patient.allergies != "")
          ? patient.allergies ?? ""
          : "";
    });

    cHasCovid.text = (patient.has_covid != null && patient.has_covid != "")
        ? patient.has_covid ?? ""
        : "";

    cCountVaccines.text = (patient.count_vaccines != null &&
            patient.count_vaccines != "" &&
            patient.count_vaccines! > 0)
        ? patient.count_vaccines.toString()
        : "";

    cHeight.text =
        (patient.height != null && patient.height != "" && patient.height! > 0)
            ? patient.height.toString()
            : "";

    cWeight.text =
        (patient.weight != null && patient.weight != "" && patient.weight! > 0)
        ...
        try {
          ...
          if (imageSelected != null)
            assets = await WebService(context)
                .uploadAsset("image", imageSelected, tokenDoctor);

          if (imageSelectedSt != null)
            assetsSt = await WebService(context)
                .uploadAsset("image", imageSelectedSt, tokenDoctor);

          PrescriptionModel prescriptionTmp = await WebService(context)
              .createPrescription(
                  (patientSelected as UserModel).id ?? "",
                  (widget.dataGuest["doctor_guest"] as UserModel).id ?? "",
                  cDiagnosis.text,
                  cEvolution.text,
                  cPrescription.text,
                  ...
                  email_doctor_guest: cEmail.text,
                  name_doctor_guest: cName.text);
          ...
          showDialog(
              ...
              builder: (contextDialog) {
                return CustomDialog(
                  "La prescripción se ha enviado correctamente",
                  "Le recomendamos registrarse en SaluMeD para una mejor experiencia.",
                  "Aceptar",
                  () {
                    Navigator.pushAndRemoveUntil(
                        context,
  ...
  searchPatient() {
    ...
    simpleLoading(context, (BuildContext contextDialog) async {
      return WebService(context)
          .getUsers(0, 0, context, tokenDoctor,
